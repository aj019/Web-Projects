    ...
    $query =(" SELECT * FROM `votes` WHERE article_id= '$article_id' AND user_id = '$user_id' " );
		...
		if($result->num_rows > 0){
			...
			if($rank==1 && $row['status']==1){
				...
				$query1="UPDATE votes SET status=1, datetime='$timestamp' WHERE article_id='$article_id' ";
				$query2="UPDATE articles SET total_downvotes = total_downvotes   WHERE article_id='$article_id' ";
				$query3="UPDATE articles SET  total_upvotes=total_upvotes + 1 WHERE article_id='$article_id' ";
				$conn->query($query1);
				$conn->query($query2);
		...
		if($days <= 2){
			if($row['df']>25){
				...
				$popular_make_query = mysql_query("UPDATE `articles` SET popular = '1' WHERE article_id = '".$article_id."' " , $conn) 
				or die("Error in query".mysql_error());			


				$ins_popular_table = mysql_query("INSERT INTO `popular` VALUES ('".$article_id."' , '".date("Y-m-d H:i:s")."' ) " , $conn) 
				or die("Error in query".mysql_error());
			}
