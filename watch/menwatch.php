...
function upload_image1(){
	...
	if (isset($name) &&isset($price)) 
	{
					...
					$query1 = "SELECT * FROM `men_watch` WHERE mName ='$name' AND mPrice = '$price'";

					$result = mysqli_query($conn,$query1);
					if(mysqli_num_rows($result) > 0){

						header('Location: '.'menwatch.php');
					}else{
						$query = "INSERT INTO `men_watch`(`mName`,`mPrice`,`mLink`,`mImageUrl`,`mImageDisplayUrl`,`mFeature1`,`mFeature2`,`mFeature3`,`mFeature4`) VALUES('".$name."','".$price."','".$link."','".$image_url."','".$image_url1."','".$feature1."','".$feature2."','".$feature3."','".$feature4."')";
						if($conn->query($query)){
							echo "Successfull";		
						}else{
