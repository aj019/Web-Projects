<?php
	...
	//	$searchtagquery =  $db->query("SELECT * FROM `articles`
	//		WHERE `article_id` IN (SELECT `article_id` FROM `tags_used` WHERE `tag` LIKE '%".$search."%' )");
		$searchtagquery = $db->query("SELECT articles.article_id , title , user_id , mv , url ,
		total_upvotes , total_downvotes , nsfw , gif , tag1 , tag2 , tag3 , credit , date_creation 
		FROM `articles` INNER JOIN `tags_used` ON articles.article_id = tags_used.article_id
		...
		$searchtitlequery = $db->query("SELECT * from `articles` WHERE `title` LIKE '%".$search."%' " );
		
		while ($row= $searchtitlequery->fetch_object()) {
	  		$articles[]= $row;
	  	}
	  	
		/*$searchtagquery = $db->query("SELECT `article_id`, `title`, `user_id`, `mv`, `url`, `total_upvotes`, `total_downvotes`, `nsfw`, `gif`, `tag1`, `tag2`, `tag3`, `credit`, `date_creation` 
		FROM `articles` , `tags_used` 
		WHERE articles.article_id = tags_used.article_id AND `tag` LIKE '%".$search."%' ");
