...
function upload_image(){
		...
		else
		{
			...
			$newname="images/women/".$image_name;
	 		$img_url = "http://youni.co.in/teetry/images/women/".$image_name;
...
function upload_image1(){
	...
	if (isset($name) &&isset($price)) 
	{
					...
					$query1 = "SELECT * FROM `womentees` WHERE wteename ='$name' AND wteeprice = '$price'";

					$result = mysqli_query($conn,$query1);
					if(mysqli_num_rows($result) > 0){

						header('Location: '.'womentee.php');
					}else{
						$query = "INSERT INTO `womentees`(`wteename`,`wteeprice`,`wteelink`,`wteedisplayimageurl`,`wteeimageurl`,`wteefeature1`,`wteefeature2`,`wteefeature3`,`wteefeature4`) VALUES('".$name."','".$price."','".$link."','".$image_url."','".$image_url1."','".$feature1."','".$feature2."','".$feature3."','".$feature4."')";
						if($conn->query($query)){
							echo "Successfull";		
						}else{
...
<body>
	...
	<form method="post" action="" enctype="multipart/form-data">
		...
		<br><br>
		<label>Upload image to display(jpeg)</label><br><br>
		<input type="file" name="image" id="image" size="40"><br><br>
		<label>Upload image to try(png)</label><br><br>
